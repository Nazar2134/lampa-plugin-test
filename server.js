(function () {
      ...
      req.native(
        ...
        function (data) {
          ...
          if (streams.length) {
            ...
            logTorrentioStreamDiagnostics(streams[0]);
          }
  ...
  function isDebridDirectUrl(url) {
    return /\.debrid\.it\//i.test(String(url || '')) || /alldebrid\.com/i.test(String(url || ''));
  }
  ...
  function logTorrentioStreamDiagnostics(stream, row) {
    stream = stream || (row && row.raw) || {};

    console.log('[TORRENTIO STREAM FULL]', stream);

    console.group('[TORRENTIO STREAM FIELDS]');
    console.log('stream.url', stream.url);
    console.log('stream.externalUrl', stream.externalUrl);
    console.log('stream.infoHash', stream.infoHash);
    console.log('stream.sources', stream.sources);
    console.log('stream.behaviorHints', stream.behaviorHints);
    console.log('stream.fileIdx', stream.fileIdx);
    ...
    if (stream.url) {
      playbackOptions.push({
        field: 'url',
        value: stream.url,
        isResolve: isTorrentioResolveUrl(stream.url),
        isDebridDirect: isDebridDirectUrl(stream.url)
      });
    }
    ...
    if (stream.externalUrl) {
      playbackOptions.push({
        ...
        isResolve: isTorrentioResolveUrl(stream.externalUrl),
        isDebridDirect: isDebridDirectUrl(stream.externalUrl)
      });
    }
    ...
    if (stream.infoHash) {
      playbackOptions.push({
        field: 'infoHash',
        value: stream.infoHash,
        fileIdx: stream.fileIdx != null ? stream.fileIdx : null,
        note: 'magnet/torrent path — not a direct http URL'
      });
    }
    ...
    if (stream.behaviorHints) {
      playbackOptions.push({
        field: 'behaviorHints',
        value: stream.behaviorHints,
        notWebReady: !!stream.behaviorHints.notWebReady,
        bingeGroup: stream.behaviorHints.bingeGroup || null,
        proxyHeaders: stream.behaviorHints.proxyHeaders || null
      });
    }

    console.group('[TORRENTIO PLAYBACK OPTIONS]');
    console.log('count', playbackOptions.length);
    playbackOptions.forEach(function (opt, index) {
      console.log('option[' + index + ']', opt);
    });
    console.log('recommended-for-web (diagnostic only):', {
      preferExternalUrl: !!stream.externalUrl,
      avoidResolveInPlayer: isTorrentioResolveUrl(stream.url),
      needsUnlockIfDebrid: isDebridDirectUrl(stream.url) || isDebridDirectUrl(stream.externalUrl),
      notWebReady: !!(stream.behaviorHints && stream.behaviorHints.notWebReady)
    });
    console.groupEnd();
  ...
  function traceTorrentioRedirectChain(url, hop, chain) {
    hop = hop || 0;
    chain = chain || [];
    ...
    if (typeof window.fetch !== 'function') {
      chain.push({ hop: hop, url: url, error: 'fetch unavailable' });
      return Promise.resolve(chain);
    }

    return window
      .fetch(url, { method: 'HEAD', redirect: 'manual', mode: 'cors' })
      .then(function (response) {
        ...
        if (response.status >= 300 && response.status < 400 && location) {
          var nextUrl = resolveRedirectLocation(url, location);
          console.log('[TORRENTIO REDIRECT]', hop, response.status, url, '→', nextUrl);
          return traceTorrentioRedirectChain(nextUrl, hop + 1, chain);
        }

        return chain;
      })
      .catch(function (err) {
        chain.push({ hop: hop, requestUrl: url, error: String((err && err.message) || err) });
        return chain;
      });
  }

  function probeTorrentioUrlDiagnostics(label, url) {
    var target = String(url || '').trim();

    console.group('[TORRENTIO URL PROBE] ' + (label || 'unknown'));
    console.log('probeUrl', target);
    console.log('isResolve', isTorrentioResolveUrl(target));
    console.log('isDebridDirect', isDebridDirectUrl(target));

    var tasks = [];

    tasks.push(
      traceTorrentioRedirectChain(target).then(function (chain) {
        console.log('[TORRENTIO REDIRECT CHAIN]', chain);
        var last = chain.length ? chain[chain.length - 1] : null;
        if (last && last.responseUrl && last.responseUrl !== target) {
          console.log('[TORRENTIO REDIRECT CHAIN] last.responseUrl', last.responseUrl);
        }
        if (last && last.location) {
          console.log('[TORRENTIO REDIRECT CHAIN] last.location', last.location);
        }
      })
    );
    ...
    if (typeof window.fetch === 'function') {
      tasks.push(
        window
          .fetch(target, { method: 'HEAD', redirect: 'follow', mode: 'cors' })
          .then(function (response) {
            console.log('[TORRENTIO URL PROBE] fetch HEAD follow', {
              ok: response.ok,
              status: response.status,
              type: response.type,
              url: response.url
          ...
          .catch(function (err) {
            console.warn('[TORRENTIO URL PROBE] fetch HEAD follow error', err);
          })
      );

      tasks.push(
        window
          .fetch(target, { method: 'GET', redirect: 'follow', mode: 'cors' })
          .then(function (response) {
            console.log('[TORRENTIO URL PROBE] fetch GET follow', {
              ok: response.ok,
              status: response.status,
              type: response.type,
              url: response.url
          ...
          .then(function (headers) {
            console.log('[TORRENTIO URL PROBE] fetch GET headers', headers);
          })
          .catch(function (err) {
            console.warn('[TORRENTIO URL PROBE] fetch GET follow error', err);
          })
      );
    }
    ...
    tasks.push(
      new Promise(function (resolve) {
        ...
        xhr.onload = function () {
          console.log('[TORRENTIO URL PROBE] xhr HEAD', {
            status: xhr.status,
            responseURL: xhr.responseURL,
            getAllResponseHeaders: xhr.getAllResponseHeaders ? xhr.getAllResponseHeaders() : ''
          });
          resolve();
        };

        xhr.onerror = function () {
          console.warn('[TORRENTIO URL PROBE] xhr HEAD error', xhr.status);
          resolve();
        };

        xhr.ontimeout = function () {
          console.warn('[TORRENTIO URL PROBE] xhr HEAD timeout');
          resolve();
        };
    ...
    return Promise.all(tasks)
      .catch(function (err) {
        console.warn('[TORRENTIO URL PROBE] task error', err);
      })
      .then(function () {
        console.groupEnd();
      });
  ...
  function runTorrentioPlaybackDiagnostics(stream, primaryUrl) {
    logTorrentioStreamDiagnostics(stream);

    var urlsToProbe = [];
    var seen = {};

    function addProbeUrl(field, value) {
      var u = String(value || '').trim();
      if (!u || seen[u]) return;
      seen[u] = true;
      urlsToProbe.push({ field: field, url: u });
    }

    if (stream) {
      addProbeUrl('stream.url', stream.url);
      addProbeUrl('stream.externalUrl', stream.externalUrl);
    }

    addProbeUrl('playUrl', primaryUrl);

    console.group('[TORRENTIO DIAGNOSTIC PROBES]');
    console.log('urlsToProbe', urlsToProbe);
    console.groupEnd();

    return urlsToProbe.reduce(function (chain, item) {
      return chain.then(function () {
        return probeTorrentioUrlDiagnostics(item.field, item.url);
      });
    }, Promise.resolve());
  }
